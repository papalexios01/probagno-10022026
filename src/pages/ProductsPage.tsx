    ...
    <>
      ...
      <Layout>
        <section className="pt-8 pb-8 sm:pt-12 sm:pb-12 bg-gradient-to-b from-muted/50 to-background">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-2xl mx-auto">
              <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-semibold mb-3 sm:mb-4">{language === 'el' ? 'Τα Προϊόντα μας' : 'Our Products'}</h1>
              <p className="text-muted-foreground text-sm sm:text-base">{language === 'el' ? 'Ανακαλύψτε τη συλλογή μας από επιλεγμένα έπιπλα μπάνιου υψηλής αισθητικής' : 'Discover our collection of high-aesthetic bathroom furniture'}</p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-6 sm:py-8 lg:py-12">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col gap-4 mb-6 sm:mb-8">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
                    ...
                    <SheetContent side="left" className="w-[85%] sm:w-[380px] overflow-y-auto"><SheetHeader><SheetTitle className="text-left flex items-center gap-2"><SlidersHorizontal className="w-5 h-5" />{language === 'el' ? 'Φίλτρα Προϊόντων' : 'Product Filters'}</SheetTitle></SheetHeader>
                      <div className="mt-6"><ProductFilters products={searchAndCategoryFiltered} allProducts={products} categories={categories} selectedCategories={selectedCategories} selectedColors={selectedColors} selectedMaterials={selectedMaterials} priceRange={priceRange} maxPrice={maxPrice} onCategoryChange={toggleCategory} onColorChange={toggleColor} onMaterialChange={toggleMaterial} onPriceChange={setPriceRange} onClearFilters={clearFilters} hasFilters={hasFilters} /></div>
                  ...
                  <div className="hidden lg:flex items-center gap-1 p-1 rounded-xl border border-input bg-background">
                    <button onClick={() => setGridCols(2)} className={cn('p-2.5 rounded-lg transition-colors', gridCols === 2 ? 'bg-primary text-primary-foreground' : 'hover:bg-muted')}><LayoutGrid className="w-4 h-4" /></button>
                    <button onClick={() => setGridCols(3)} className={cn('p-2.5 rounded-lg transition-colors', gridCols === 3 ? 'bg-primary text-primary-foreground' : 'hover:bg-muted')}><Grid3X3 className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
